...
vertex_t *GraphShortestBFS(const GraphPtr g,
                           vertex_t start,
                           vertex_t end,
                           int *path_len)
{
    if (!g || !path_len || start < 0 || start >= GraphV(g) || end < 0 || end >= GraphV(g))
    {
        return NULL;
    }
    ...
    vertex_t *edge_to = malloc(V * sizeof(vertex_t));
    vertex_t *queue_buf = malloc(V * sizeof(vertex_t));
    int q_head = 0, q_tail = 0;

    // initialize
    visited[start] = true;
    queue_buf[q_tail++] = start;

    bool found = false;
    while (q_head < q_tail && !found)
    {
        vertex_t v = queue_buf[q_head++];
        int adj_size = 0;
        const vertex_t *adj = GraphAdj(g, v, &adj_size);
        for (int i = 0; i < adj_size; i++)
        {
            vertex_t w = adj[i];
            if (!visited[w])
    ...
    vertex_t *rev = malloc(V * sizeof(vertex_t));
    int count = 0;
    for (vertex_t x = end; x != start; x = edge_to[x])
    {
        rev[count++] = x;
    }
    rev[count++] = start;
    ...
    // reverse into result
    vertex_t *result = malloc(count * sizeof(vertex_t));
    for (int i = 0; i < count; i++)
    {
        result[i] = rev[count - 1 - i];
    }
    free(rev);
    free(visited);
    free(edge_to);
